...
When a Move leaves the Turn open, the player is offered End Turn. They cannot End Turn with zero Moves this Turn unless Match Settings allow it (default off). End Turn is illegal while that Side is in check, unless a Mode changes that. The Side’s clock runs for the whole Turn, including extra Moves. Timeout is an immediate loss even mid-Move, unless a Mode says otherwise.
